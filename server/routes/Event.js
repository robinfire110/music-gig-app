...
router.get("/", async (req, res) => {
  try {
    const events = await models.Event.findAll({
      include: [models.Instrument, models.Address, models.User],
    });
    res.json(events);
  } catch (error) {
...
router.post("/", async (req, res) => {
  try {
    ...
    const newStatus = await models.UserStatus.create({
    ...
    if (data.instruments) {
      for (const instrument of data.instruments) {
        ...
        //Add if found
        if (instrumentId) {
          var newInstrument = await models.EventInstrument.create({
...
router.post("/users/:event_id/:user_id", async (req, res) => {
  try {
    ...
    //Get event
    const newStatus = await models.UserStatus.findOrCreate({
      user_id: user_id,
      event_id: event_id,
      status: data?.status,
    });
  ...
  try {
    ...
    if (data.instruments) {
      for (const instrument of data.instruments) {
        ...
        //Add if found
        if (instrumentId) {
          newInstrument = await models.EventInstrument.findOrCreate({
            where: { instrument_id: instrumentId, event_id: id },
          });
          newInstrumentArray.push(newInstrument);
        } else {
          console.log(
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (event) {
      ...
      if (data.start_time || data.end_time) {
        event_hours = models.getEventHours(
          data.start_time ? data.start_time : event.start_time,
          data.end_time ? data.end_time : event.end_time,
        );
      ...
      if (data.address) {
        const address = await models.Address.findOne({
          where: { address_id: event.Address.address_id },
        });
      ...
      if (data.instruments) {
        ...
        for (const instrument of data.instruments) {
          ...
          if (instrumentId) {
            newInstrument = await models.EventInstrument.findOrCreate({
              where: { instrument_id: instrumentId, event_id: event.event_id },
            });
            newInstrumentArray.push(newInstrument);
          } else {
            console.log(
...
router.put("/users/:event_id/:user_id", async (req, res) => {
  try {
    const data = req.body;
    const { event_id, user_id } = req.params;
    const status = await models.UserStatus.findOne({
    ...
    if (status) {
      ...
      res
        .status(404)
        .send(
          `No user_id ${user_id} found in relation to event of event_id ${event_id} found.`,
        );
    }
  } catch (error) {
    res.status(500).send(error.message);
  ...
  try {
    ...
    if (status) {
      ...
      res
        .status(404)
        .send(
          `No user_id ${user_id} associated with event of event_id ${event_id} found.`,
        );
    }
  } catch (error) {
    res.status(500).send(error.message);
...
router.delete("/instrument/:user_id/:instrument_id", async (req, res) => {
  try {
    ...
    const instrument = await models.EventInstrument.findOne({
      where: { event_id: event_id, instrument_id: instrument_id },
    });
    ...
    if (instrument) {
      ...
      res
        .status(404)
        .send(
          `No instrument of event_id ${event_id} and instrument_id ${instrument_id} found.`,
        );
    }
  } catch (error) {
    res.status(500).send(error.message);

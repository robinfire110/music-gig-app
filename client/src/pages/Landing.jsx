...
function Landing() {
    ...
    useEffect(() => {
        if (cookies.jwt)
        {
            ...
            {
                //Get user
                axios.get(`${getBackendURL()}/api/account`, {withCredentials: true}).then(async res => {
                    //Get data based on user
                    if (res.data?.user)
                    {
                        //Get user data
                        axios.get(`${getBackendURL()}/api/user/id/${res.data.user.user_id}`).then(async res => {
                            ...
                            {
                                axios.get(`${getBackendURL()}/api/event/instrument/${instrumentSearch.join("|")}?sort=true&limit=${25}`).then(res => {
                                    ...
                                    //Sort by location
                                    axios.get(`${getBackendURL()}/api/api/distance_matrix/${userData.zip}/${zipList.join("|")}`).then(res => {
    ...
    useEffect(() => {
        if (getRecent)
        {
            ...
            axios.get(`${getBackendURL()}/api/event/recent/${recentNum}`).then(res => {
